...
# read the file and check what the current classes are
# select prof or TA, start time, end time, location

def extract_class_names(file_name):
    class_names = []


    # Construct the relative path
    script_dir = os.path.dirname(__file__)  # Directory of the script
    file_path = os.path.join(script_dir, file_name)  # Path to the file

    if not os.path.exists(file_path):
        return class_names  # Return empty list if file does not exist
...
class InputApp(App):
    ...
    def compose(self) -> ComposeResult:
        ...
        self.inputs["office_hours_location"] = class_location_input

        yield Label("Office hours location")
